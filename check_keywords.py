...
if not os.path.isfile(keyword_file): # 파일이 없을 경우
    raise FileNotFoundError(f"키워드 파일 없음: {keyword_file}")

with open(keyword_file, "r", encoding="utf-8") as f: # keyword_file 을 utf-8 방식으로 인코딩해서 '읽는다("r" : read, 읽기 모드)
    keywords = [line.strip() for line in f if line.strip()] # 텍스트 파일의 한 라인 전체를 키워드로 해서 읽는다. 

print(f"🔑 읽은 키워드: {keywords}\n") # 어떤 키워드들을 읽었는지 확인

# 키워드별 총합
keyword_counts = {kw: 0 for kw in keywords} # dict 방식, 각 속성에 접근하기 쉬워 유용

# 전체 메시지 개수 (확인차 설정)
total_count = 0 
total_successed_count = 0 # S 메시지 개수
total_failed_count = 0 # F 메시지 개수
matched_total_count = 0 # 문자 내에서 키워드를 찾은 횟수
...
print(f"키워드 매칭된 개수 총합: {matched_total_count:,}개")
# print(f"❗ 키워드에 걸리지 않은 메시지: {unmatched_count:,}개\n")
